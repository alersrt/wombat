...
func NewAesGcmCipher(key []byte) (*AesGcmCipher, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cipher: new: %v", err)
	}
...
func (a *AesGcmCipher) Encrypt(plaintext string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cipher: encrypt: %v", err)
	}
...
func (a *AesGcmCipher) Decrypt(ciphertext []byte) (string, error) {
	...
	if len(ciphertext) < nonceSize {
		return "", fmt.Errorf("cipher: decrypt: size mismatch")
	}
	nonce, ciphertext := ciphertext[:nonceSize], ciphertext[nonceSize:]
	plaintextBytes, err := a.gcm.Open(nil, nonce, ciphertext, nil)
	if err != nil {
		return "", fmt.Errorf("cipher: decrypt: %v", err)
	}
